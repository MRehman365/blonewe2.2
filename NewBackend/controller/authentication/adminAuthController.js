...
class AdminsAuthController {
  ...
  updatepassword = async (req, res) => {
    try {
      ...
      const adminDetails = await userAuthModal.findOne({ email });
  ...
  getAdminDetails = async (req, res) => {
    try {
      const adminDetails = await adminBioModel.find({});
  ...
  updateDetails = async (req, res) => {
    try {
      const { adminId, email, username, bio, image, fullName, phone } =
        req.body;
      ...
      if (!authDetails) {
        return res
          .status(404)
          .json({ message: "Admin authentication details not found" });
      }
